...
> Source: https://labuladong.online/algo/en/frequency-interview/print-prime-number/
...
This is a basic fact in number theory: if a number nnn is not prime, then it must have a factor less than or equal to n\sqrt{n}n​.
...
Now the time complexity of the `isPrime` function is O(N)O(\sqrt{N})O(N​). **But actually, we do not need this function to implement`countPrimes`**. I only want you to understand the idea of `sqrt(n)` here, because we will use it again later.
...
![diagram](https://labuladong.online/images/algo/prime/1.gif)
...
The part in brackets is the sum of the reciprocals of primes. The final result is O(N∗log⁡log⁡N)O(N * \log\log N)O(N∗loglogN). If you are interested, you can look up the proof of this time complexity.
