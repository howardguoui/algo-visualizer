...
> Source: https://labuladong.online/algo/en/data-structure/prefix-sum/
...
This solution runs a for loop every time `sumRange` is called. The time complexity is O(N)O(N)O(N). Since `sumRange` may be called very often, this is not efficient.
...
In this way, `sumRange` only needs one subtraction, no loop, so the worst-case time complexity is constant O(1)O(1)O(1).
...
![diagram](https://labuladong.online/images/lc/uploads/2021/03/14/sum-grid.jpg)
...
The problem is from [LeetCode 304. Range Sum Query 2D - Immutable](<https://leetcode.com/problems/range-sum-query-2d-immutable/>).
...
![diagram](https://labuladong.online/images/algo/presum/5.jpeg)
...
Moreover, prefix sum arrays are often combined with other data structures or algorithmic techniques. I will explain these combinations in [High-Frequency Prefix Sum Exercises](</en/algo/problem-set/perfix-sum/>) along with relevant exercises.
...
If an element in the original array changes, the values in the `preSum` array after that element become invalid, requiring O(n)O(n)O(n) time to recalculate the `preSum` array, which is similar to the brute-force method.

**Second Limitation: The prefix sum technique is only applicable in scenarios with inverse operations.**

For example, in summation scenarios, if you know x+6=10x + 6 = 10x+6=10, you can deduce x=10−6=4x = 10 - 6 = 4x=10−6=4. Similarly, in product scenarios, if you know x∗6=12x * 6 = 12x∗6=12, you can deduce x=12/6=2x = 12 / 6 = 2x=12/6=2. This is known as having an inverse operation, which allows the use of the prefix sum technique.

However, some scenarios do not have inverse operations. For example, in maximum value scenarios, if you know max(x,8)=8max(x, 8) = 8max(x,8)=8, you cannot deduce the value of xxx.
