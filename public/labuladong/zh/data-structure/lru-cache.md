...
现在你应该理解 LRU（Least Recently Used）策略了。当然还有其他缓存淘汰策略，比如不要按访问的时序来淘汰，而是按访问频率（LFU 策略）来淘汰等等，各有应用场景。本文讲解 LRU 算法策略，我会在 [LFU 算法详解](</zh/algo/frequency-interview/lfu/>) 中讲解 LFU 算法。
...
![diagram](https://labuladong.online/images/algo/lru/4.jpg)
...
如果对链表的操作不熟悉，可以看前文 [手把手带你实现双链表](</zh/algo/data-structure-basic/linkedlist-basic/>)。
...
![diagram](https://labuladong.online/images/algo/lru/put.jpg)
...
你也可以用 Java 的内置类型 `LinkedHashMap` 或者 [手把手带你实现哈希链表](</zh/algo/data-structure-basic/hashtable-with-linked-list/>) 实现的 `MyLinkedHashMap` 来实现 LRU 算法，逻辑和之前完全一致：
...
至此，LRU 算法就没有什么神秘的了。更多数据结构设计相关的题目参见 [数据结构设计经典习题](</zh/algo/problem-set/ds-design/>)。
