...
你可以点开下面的可视化面板，多次点击 `diff[i] = nums[i] - nums[i - 1]` 这行代码就可以看到 `diff` 数组的构建过程，再多次点击 `df.increment` 这行代码可以看到 `diff` 数组的操作：
...
题目来源：[力扣 1109. 航班预订统计](<https://leetcode.cn/problems/corporate-flight-bookings/>)。
...
题目来源：[力扣 1094. 拼车](<https://leetcode.cn/problems/car-pooling/>)。
...
差分数组和前缀和数组都是比较常见且巧妙的算法技巧，分别适用不同的场景，而且是会者不难，难者不会。
