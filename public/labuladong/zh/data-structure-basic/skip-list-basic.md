...
在 [链表基础](</zh/algo/data-structure-basic/linkedlist-basic/>) 中我们说到，在单链表中增删查改**指定索引** 的元素所需的时间复杂度是 O(N)O(N)O(N)。
...
有一种方式是借助键值映射，用 O(1)O(1)O(1) 的时间直接拿到目标节点，避免了遍历查找的时间消耗，这个思路在后面的 [哈希链表（LinkedHashMap）](</zh/algo/data-structure-basic/hashtable-with-linked-list/>) 中会详细介绍。
