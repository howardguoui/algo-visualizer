...
> Source: https://labuladong.online/zh/algo/frequency-interview/print-prime-number/
...
现在，`isPrime` 函数的时间复杂度降为 O(sqrt(N))O(sqrt(N))O(sqrt(N))，**但是我们实现`countPrimes` 函数其实并不需要这个函数**，以上只是希望读者明白 `sqrt(n)` 的含义，因为等会还会用到。
