...
认证中心(op.company.com)应用 A(mail.company.com)用户浏览器认证中心(op.company.com)应用 A(mail.company.com)用户浏览器没有 Session Cookie没有 Session Cookie 显示登录页面设置 Session Cookie1\. 访问企业邮箱2\. 未登录，重定向到认证中心3\. 访问认证中心4\. 输入用户名和密码5\. 验证凭据6\. 颁发身份凭证，重定向回应用 A7\. 访问应用 A，带上身份凭证8\. 验证凭证："这个凭证是真的吗？"9\. 返回用户身份信息10\. 允许访问邮箱
...
认证中心(op.company.com)应用 B(crm.company.com)用户浏览器认证中心(op.company.com)应用 B(crm.company.com)用户浏览器没有 Session Cookie检查 Session Cookie用户已登录！1\. 访问 CRM 系统2\. 未登录，重定向到认证中心3\. 访问认证中心4\. 直接颁发新的身份凭证重定向回应用 B（无需输入密码）5\. 访问应用 B，带上身份凭证6\. 验证凭证7\. 返回用户身份信息8\. 允许访问 CRM 系统
