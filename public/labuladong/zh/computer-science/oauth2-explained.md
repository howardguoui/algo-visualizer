...
Google Drive资源服务器Google授权服务器ExampleNote服务器用户浏览器Google Drive资源服务器Google授权服务器ExampleNote服务器用户浏览器1\. 点击"导入 Google Drive 文件"2\. 重定向到 Google 授权页面3\. 访问授权页面4\. 显示授权确认页面5\. 用户点击"同意"6\. 重定向回 ExampleNote(带上授权码 code)7\. 浏览器访问回调地址(URL 中包含 code)8\. 用 code + client_secret换取 Access Token9\. 返回 Access Token10\. 用 Access Token请求用户的文件列表11\. 返回文件列表12\. 显示导入的文件
