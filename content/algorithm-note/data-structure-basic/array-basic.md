...
2、因为数组的名字 `$就指向整块内存的首地址，所以数组名 `$ 就是一个指针。你直接取这个地址的值，就是第一个元素的值。也就是说，`*arr` 的值就是 `arr[0]`，即第一个元素的值。
...
这个问题很好，但并不是每次增加元素的时候都会触发扩容，所以扩容的复杂度要用「均摊时间复杂度」来分析，这个概念我在 [时空复杂度分析方法](</zh/algo/essential-technique/complexity-analysis/>) 中有详细的讲解，这里就不展开了。
