...
遍历图的「节点」和「边$`visited` 数组在前序位置做标记，避免重复遍历；遍历图的「路径$ `onPath` 数组在前序位置标记节点，在后序位置撤销标记。
...
如果我们不标记遍历过的节点，那么从 `1` 开始遍历，会走到 `$`1`，再走到 `$ `1`，如此 `1->2->1->2->...` 无限递归循环下去。
...
我们之前讲解 [二叉树的遍历](</zh/algo/data-structure-basic/binary-tree-traverse-basic/>) 时说，二叉树的遍历函数时间复杂度是 $O(N)$，其中 $N$ 是节点的总数。
...
上面遍历所有节点的代码用一个一维的 `$数组记录已经访问过的节点，确保每个节点只被遍历一次；那么最简单直接的实现思路就是用一个二维的 `$ 数组来记录遍历过的边（`visited[u][v]` 表示边 `u->v` 已经被遍历过），从而确保每条边只被遍历一次。
...
显然，使用二维 `$数组并不是一个很高效的实现方式，因为需要创建二维 `$ 数组，这个算法的时间复杂度是 $O(E + V^2)$，空间复杂度是 $O(V^2)$，其中 $E$ 是边的数量，$V$ 是节点的数量。
