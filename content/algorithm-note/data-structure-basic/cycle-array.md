...
这样，当我们在数组头$`start` 索引，而在数组尾$ `end` 索引。
...
当然，上面实现的这个环形数组只提供了 `addFirst, removeFirst, addLast, removeLast` 这几个方法，并没有提供 [我们之前实现的动态数组](</zh/algo/data-structure-basic/array-implement/>) 的某些方法，比如删除指定索引的元素，获取指定索引的元素，在指定索引插入元素等等。
