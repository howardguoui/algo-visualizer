...
但是我们可以改用前文 [环形数组技巧](</zh/algo/data-structure-basic/cycle-array/>) 中实现的 `CycleArray` 类，这个数据结构在头部增删元素的时间复杂度是 $O(1)$，符合栈的要求。

你直接调用 `CycleArray` 的 `addFirst` 和 `removeFirst` 方法实现栈的 API 就行，我这里就不写了。

## ¶用数组实现队列

有了前文 [环形数组](</zh/algo/data-structure-basic/cycle-array/>) 中实现的 `$类，用数组作为底层数据结构实现队列就不难了吧。直接复用我们实现的 `$，就可以实现标准队列了。当然，一些编程语言也有内置的环形数组实现，你也可以自行搜索使用：
