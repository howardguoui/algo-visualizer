...
图中节点 `1` 和节点 `3` 之间有一条无向边，这说明可以从节点 `1` $`3`，也可以从节点 `3`$ `1`。
...
考虑 $E$ 的最大值，图中的每个节点最多可以有 V$条边与其他 V$ 个节点相连，所以最多能有的边数为 E=V(V−1)/2≈V2E = V(V-$\approx V^2E=V(V−$≈V2。

如果几乎每两个节点之间都有一条边，即 $E$ 接近 V2V^2V2，我们说这幅图是 **稠密图（Dense Graph）** ；如果只有很少的边，即 $E$ 远小于 V2V^2V2，我们说这幅图是 **稀疏图（Sparse Graph）** 。

## ¶子图

在图论中，子图是一个重要的基本概念。

**子图 (Subgraph)** ：如果图 G′G'G′ 的所有节点和边都包含在图 $G$ 中，则称 G′G'G′ 是 $G$ 的一个子图。简单来说，子图是从原图中删除一些节点和边后得到的图。
