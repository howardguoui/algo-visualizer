...
因为我的 [算法可视化面板](</zh/algo/intro/visualize/>) 目前只支持 J$语言，所以这个教程不仅面向 J$ 初学者，也面向其他编程语言的读者，帮助大家快速上手算法可视化面板。
...
我的可视化代码中的顶层函数一般是用 `var` 声明的，这是因为 LeetCode 上给的 JS 函数签名都是用 `$声明的，所以我也沿用 `$ 了，不过你非要改成 `let` 也是没问题的。
...
比如 JavaScript 中的 `null` 和 `$都表示空值，`null ==$ 是 `true`，但是 `null === undefined` 是 `false`。
...
通俗来讲，J$中的对象就可以理解为是哈希表，因为 J$ 对象就是若干键值对。不过 ES6 中引入了 `$类型，所以我们就规范一些，用 `$ 类型来创建哈希表。
