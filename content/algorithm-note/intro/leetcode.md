...
比如这道题，下面补充说明了 $的长度范围，和其中元素的大小范围，还有一个很重要的点，就是告诉你 **$ 中并没有重复元素**。
...
那么如果题目有一些软限制，比方说$`long` 类型，禁止你使用标准库之类的约束，其实它只是那么说一说，没有办法真的$。
...
比方说上面的例子，C++ 代码中的 `traverse` 函数参数中一定要用传引用的方式 `vector<int>& res`，如果用传值的方式 `$res`，那么每次递归调用都会拷贝一个新的 `$` 对象，不仅会导致效率大幅下降，甚至可能导致答案错误。
