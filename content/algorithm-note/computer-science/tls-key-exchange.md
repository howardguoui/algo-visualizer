...
模幂运算有一个特性：计算很容易，但逆运算（离散对数）很难：
  ...
  * 已知 $g$、$x$、$p$，计算 gx mod pg^x \bmod pgxmodp 的结果 $y$ 很容易。
...
要从 A=8A=8A=8 反推出 $a$，需要解决：5a mod 23=85^a \bmod 23 = 85amod23=8，求 $a$。
...
前面说了，从 gx mod pg^x \bmod pgxmodp 的结果反推 $x$ 是极其困难的。
...
传统 DH 算法基于模幂运算 gx mod pg^x \bmod pgxmodp，其中 $x$ 是双方各自生成的临时私钥。

椭圆曲线版本基于椭圆曲线点乘 x⋅Gx \cdot Gx⋅G，其中 $x$ 同样是私钥，$G$ 是椭圆曲线上的一个基点（公开参数）。
