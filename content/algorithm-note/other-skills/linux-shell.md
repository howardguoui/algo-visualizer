...
> Source: https://labuladong.online/zh/algo/other-skills/linux-shell/
...
前文 [Linux文件描述符](</zh/algo/other-skills/linux-process/>) 说过，管道符和重定向符是将数据作为程序的标准输入，而`$(cmd)`是读取`cmd`命令输出的数据作为参数。
...
每一个命令行终端都是一个 s$进程，你在这个终端里执行的程序实际上都是这个 s$ 进程分出来的子进程。正常情况下，s$进程会阻塞，等待子进程退出才重新接收你输入的新的命令。加上`&`号，只是让 s$ 进程不再阻塞，可以继续响应你的新命令。但是无论如何，你如果关掉了这个 shell 命令行端口，依附于它的所有子进程都会退出。
...
$的行为可以测试，使用`set -x`命令，会开启$ 的命令回显，你可以通过回显观察 shell 到底在执行什么命令：

![](/images/algo/linuxshell/1.png)
...
**当使用`sudo` 时，系统会使用 `$这个文件中规定的该用户的权限和环境变量**，而这个脚本在 `$ 环境变量目录中当然是找不到的。
...
我一般不使用 bash 作为终端，我给大家推荐一款很好用的 $终端叫做 zsh，这也是我自己使用的$。这款终端还可以扩展各种插件，非常好用，具体配置方法可自行搜索。
...
我们安装某些软件的时候，可能有交互式的提问：
    ...
    $ sudo apt install $X$
    ...
    $X$ will use 996 MB disk space, continue? [y/n]
...
如果你读过前文 [Linux 文件描述符](</zh/algo/other-skills/linux-process/>)，就知道其原理很简单：

你单独运行一下 `yes` 命令，发现它就是打印出一大堆字符 y，通过管道把输出和 `$的标准输入相连接，如果 `$ 又提出无聊的问题，就会从标准输入读取数据，也就会读取到一个 y 和换行符，和你手动输入 y 确认是一个效果。

**2、特殊变量`$?` 记录上一次命令的返回值**。

在 Linux shell 中，遵循 C 语言的习惯，返回值为 0 的话就是程序正常退出，非 0 值就是异常退出出。读取上一次命令的返回值在平时使用命令行时感觉没什么用，但是如果你想编写一些 shell 脚本，知道返回值非常有用。

举个例子，比如我需要批量给很多 markdown 文件添加页脚，有的文章已经有了页脚，大部分都没有。

为了防止重复添加，我必须知道一个 md 文件下方是否已添加页脚，这时候就可以使用 `$?` 变量配合 `grep` 命令做到：
