...
> Source: https://labuladong.online/zh/algo/other-skills/linux-process/
...
我们用 C 语言写一个 h$程序，编译后得到一个可执行文件，在命令行运行就可以打印出一句 h$ world，然后程序退出。在操作系统层面，就是新建了一个进程，这个进程将我们编译出来的可执行文件读入内存空间，然后执行，最后退出。
...
其中比较有意思的是 `mm` 指针和 $指针。`mm` 指向的是进程的虚拟内存，也就是载入资源和可执行文件的地方；$ 指针指向一个数组，这个数组里装着所有该进程打开的文件的指针。
...
明白了这个原理，**输入重定向** 就很好理解了，程序想读取数据的时候就会去 `$读取，所以我们只要把 `$ 指向一个文件，那么程序就会从这个文件中读取数据，而不是从键盘：
...
为什么说 L$中线程和进程基本没有区别呢，因为从 L$ 内核的角度来看，并没有把线程和进程区别对待。
