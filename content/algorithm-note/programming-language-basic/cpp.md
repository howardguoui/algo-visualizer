...
当然，C 语言的 `printf` 函数也可以用，但是 `$更加方便，所以我们一般用 `$。
...
for 和 $都可以用来做循环，for 循环一般用于已知循环次数的情况，$ 循环一般用于未知循环次数的情况。
...
一般来说，当我们想在头部增删元素时会使用双链表，因为它在头部增删元素的效率比 `$高。但我们通过索引访问元素，这种场景下我们会使用 `$。
...
在上述代码中，$的值被修改为 10，因为我们传递的是$ 的引用，函数内对 `x` 的修改直接影响了 `num`。
