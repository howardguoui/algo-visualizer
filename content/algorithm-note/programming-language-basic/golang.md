...
> Source: https://labuladong.online/zh/algo/programming-language-basic/golang/
...
Golang 标准库 `container/heap` 提供了堆的操作，但需要自己实现 `heap.Interface` 接口。虽然比其他语言繁琐一些，但可以把下面的模板代码复制过去直接使用。
...
在算法题中优先级队列被大量使用，比如 Dijkstra 最短路径算法、合并 K 个有序链表等经典问题都需要用到它。
    ...
    // intHeap 实现 heap.Interface，默认小顶堆
    type intHeap []int
...
一般会推荐使用 `map[int]$来模拟哈希集合，因为 `$ 不会占用额外的内存空间，而 `bool` 类型会占用一个字节的内存空间。
