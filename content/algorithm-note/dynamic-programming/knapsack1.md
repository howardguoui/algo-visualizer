...
> Source: https://labuladong.online/zh/algo/dynamic-programming/knapsack1/
...
解决这个问题没有什么排序之类巧妙的方法，只能穷举所有可能，根据我们 [动态规划详解](</zh/algo/essential-technique/dynamic-programming-framework/>) 中的套路，直接走流程就行了。
...
为什么要这么定义？因为这样可以找到状态转移关系，或者说这就是背包问题的特殊定义方式，你当做套路记下来就行，未来遇到类似背包问题的场景，都可以这样定义试一试。
...
**如果你没有把这第`i` 个物品装入背包**，那么很显然，最大价值 `dp[i$应该等于 `dp[i-1$，继承之前的结果。
...
我们可以发现，`dp[i][..]` 只和 `dp[i-1][..]` 有关，所以可以 [对动态规划进行空间压缩](</zh/algo/dynamic-programming/space-optimization/>)，优化空间复杂度。

具体来说，我们不需要维护一个 `N` 行的二维数组，只需要维护一个 2 行的二维数组 `dp[2][W+1]` 即可。在计算第 `i` 行的值时，只需要利用 `(i-1) % 2` 行的值即可。
