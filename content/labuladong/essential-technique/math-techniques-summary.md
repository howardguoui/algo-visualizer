...
ab={(ab/2)2,当 b 为偶数a⋅ab−1,当 b 为奇数a^b = \begin{cases} (a^{b/2})^2, & \text{当 b 为偶数} \\\ a \cdot a^{b-1}, & \text{当 b 为奇数} \end{cases}ab={(ab/2)2,a⋅ab−1,​当 b 为偶数当 b 为奇数​

每次可以将规模减半，因此时间复杂度为 O(log⁡b)O(\log b)O(logb)。
