...
注意装饰器模式和前面讲到的 [适配器模式](</zh/algo/design-pattern/adapter/>) 的区别：
    ...
    public class BusinessHandler implements Handler {
        ...
        public void handle(Request request, Response response) {
            ...
            if ("/api/user".equals(request.getPath())) {
                ...
                response.setBody("{ \"name\": \"Alice\", \"email\": \"[[email protected]](</cdn-cgi/l/email-protection>)\" }");
            } else {
...
输出如下：
    ...
    Final Response: Response [statusCode=200, body='{ "name": "Alice", "email": "[[email protected]](</cdn-cgi/l/email-protection>)" }']
...
其实这就是 [单链表的递归遍历](</zh/algo/data-structure/palindrome-linked-list/>) 中你把代码写到前序位置，执行顺序就是从前往后；你把代码写到后序位置，执行顺序就是从后往前。
