...
const components: Components = {
    ...
    return (
      <BlockCodeCtx.Provider value={true}>
        <div className="my-4 rounded-xl overflow-hidden border border-slate-200 dark:border-slate-700">
          {lang && (
            <div className="bg-slate-100 dark:bg-slate-800 px-4 py-2 text-xs text-slate-500 dark:text-slate-400 font-mono border-b border-slate-200 dark:border-slate-700 flex items-center gap-2">
          ...
          <pre className="bg-slate-50 dark:bg-slate-900 p-4 overflow-x-auto text-sm font-mono text-gray-800 dark:text-slate-200 leading-6">
            {children}
          </pre>
        </div>
  ...
  code: ({ children }) => {
    ...
    return (
      <code className="bg-blue-50 dark:bg-slate-800 text-blue-700 dark:text-blue-300 px-1.5 py-0.5 rounded text-sm font-mono">
        {children}
      </code>
    )
  },
